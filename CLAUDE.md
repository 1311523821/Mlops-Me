...
- **文件顶部**：一行简述模块用途（如 `"""声纳检测模型 — LVNet 多帧分割网络包装器"""`）
- **公开类/函数**：docstring 说明入参、返回值、关键行为
- **关键决策点**：行内注释解释"**为什么**这么做"，而不是重复代码"做了什么"
  - 好：`# PixelUnshuffle(2) 将空间信息折叠到通道，保持感受野的同时降低分辨率`
  - 差：`# 初始化 x = 0`
- **非直观操作**：维度变换（`permute`/`reshape`）、设备转移、数值缩放等处必须注释意图
- **禁止**：无意义占位注释、大段废弃代码、中英文混用在同一句
